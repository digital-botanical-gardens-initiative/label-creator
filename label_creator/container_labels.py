...
def main(container_labels_window: tk.Toplevel, root: tk.Tk, label_info: tk.Label, label_info_2: tk.Label) -> None:
    ...
    request_url = collection_url + f"?filter[{field_name}][_starts_with]=container_&&limit=1"
    ...
    template["container_id"] = ["container_" f"{first_number + i:06d}" for i in range(number)]
    ...
    values = ["container_" f"{first_number + i:06d}" for i in range(number)]
    ...
    if response.status_code == 200:
        ...
        container_labels_window.destroy()
        root.destroy()
...
def create_big_labels_pdf(values: list, output_folder: str) -> None:
    ...
    pdf_path = f"{output_folder}/big_container_labels_L4736.pdf"
    ...
    for group in value_groups:
        ...
        for i, value in enumerate(group):
            ...
            # Draw the label text
            pdf.setFont("Helvetica", font_size - 1.5)
            pdf.drawString(pos_x + 0.1 * cm, pos_y + 1.2 * cm, value[:9])
            pdf.setFont("Helvetica", font_size)  # Reduce font size for the second line
            pdf.drawString(pos_x + 0.1 * cm, pos_y + 0.45 * cm, value[9:])
...
def create_medium_labels_pdf(values: list, output_folder: str) -> None:
    ...
    # Set up the PDF canvas
    pdf_path = f"{output_folder}/medium_container_labels_L4732.pdf"
    pdf = canvas.Canvas(pdf_path, pagesize=A4)
    ...
    for group in value_groups:
        ...
        for i, value in enumerate(group):
            ...
            # Draw the label text
            pdf.setFont("Helvetica", font_size - 1.5)
            pdf.drawString(pos_x + 0.1 * cm, pos_y + 1.0 * cm, value[:9])
            pdf.setFont("Helvetica", font_size)  # Reduce font size for the second line
            pdf.drawString(pos_x + 0.1 * cm, pos_y + 0.45 * cm, value[9:])
...
def create_small_labels_pdf(values: list, output_folder: str) -> None:
    # Splitting the values into groups of 189 (number of labels per page)
    value_groups = [values[i : i + 189] for i in range(0, len(values), 189)]

    # Set up the PDF canvas
    pdf_path = f"{output_folder}/small_container_labels_L4731.pdf"
    pdf = canvas.Canvas(pdf_path, pagesize=A4)
    ...
    for group in value_groups:
        ...
        for i, value in enumerate(group):
            ...
            pdf.drawString(pos_x + 0.2 * cm, pos_y + 0.8 * cm, value[:9])
            pdf.setFont("Helvetica", font_size)  # Reduce font size for the second line
            pdf.drawString(pos_x + 0.2 * cm, pos_y + 0.4 * cm, value[9:])
